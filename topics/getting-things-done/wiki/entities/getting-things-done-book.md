---
title: Getting Things Done (book)
type: entity
tags: [book, gtd, productivity]
created: 2026-04-08
updated: 2026-04-08
sources: [raw/Getting Things Done - David Allen.pdf, raw/Getting Things Done by David Allen.pdf, raw/ef8af1_7fb55adc8de9488cade9a7da39003ce1.pdf]
---

*Getting Things Done* is the primary textual source for the GTD system. Multiple copies of the book are present in `raw/`, and together they provide the strongest reference base for Groundloop's workflow assumptions.

## Why It Matters

- it defines the canonical GTD workflow
- it explains the role of projects, next actions, and regular review
- it supports the design choice to build a constrained pipeline instead of an autonomous agent

## Available Source Records

- [Getting Things Done (calibre 2023 PDF)](../sources/getting-things-done-book-calibre-2023.md)
- [Getting Things Done (calibre 2021 PDF)](../sources/getting-things-done-book-calibre-2021.md)
- [Getting Things Done (PDFDrive 2019 PDF)](../sources/getting-things-done-book-pdfdrive-2019.md)

## Related Pages

- [[gtd]]
- [[david-allen]]
